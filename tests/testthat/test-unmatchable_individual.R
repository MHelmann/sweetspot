test_that("Create_match_sets works correctly", {
  ...
  result <- create_match_sets(treated, risk_scores, scaled_effect, control_treated_ratio, response)
...
test_that("create_match_sets handles non-binary treated vector", {
  ...
  scaled_effect <- risk_scores # probability
  control_treated_ratio <- 1
  response <- c(0, 1, 0, 1, 1)

  # Expect an error
  expect_error(
    create_match_sets(treated, risk_scores, scaled_effect, control_treated_ratio, response),
